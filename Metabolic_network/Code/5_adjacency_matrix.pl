...
#this script reads a network cratet by 4_network_constuction.pl and writes an adjacency matrix
#use: ./5_adjacency_matrix.pl ../Results/network.reactions_slim.filter_40
	...
	open my $OUT, "> ../Results/EC-EC_adjacency.matrix";
...
#_____________________#Print EC numbers in global metabolic network keeped after filter to file________________________

	print "Printing reactions to file\n";
	my @ecordered;
	for my $i (0 .. $#adjacency_matrix) {
		$ecordered[$i] = $adjacency_matrix[$i][0];
	}
	@ecordered = sort @ecordered;
	open my $ECLIST, "> ../Results/eclist";
	for my $i (0 .. $#ecordered) {
		print $ECLIST "$ecordered[$i]\n";
	}
	close $ECLIST;	
